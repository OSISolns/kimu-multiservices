      ...
      <section className="py-16 bg-gradient-to-br from-blue-50 to-white">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="relative flex flex-col items-center">
            {/* Enhanced Slide */}
            {vehicles.length > 0 && vehicles[0] && vehicles[0].image ? (
              <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden transition-all duration-700 ease-in-out">
                ...
                <div className="relative h-80 bg-gradient-to-br from-blue-50 to-gray-50">
                  ...
                  <div className="absolute top-4 right-4">
                    <div className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold shadow-lg ${
                      vehicles[0]?.isAvailable 
                        ? 'bg-green-100 text-green-700' 
                        : 'bg-red-100 text-red-700'
                    }`}>
                      <div className={`w-2 h-2 rounded-full ${
                        vehicles[0]?.isAvailable ? 'bg-green-500' : 'bg-red-500'
                      }`}></div>
                      {vehicles[0]?.isAvailable ? 'Available' : 'Rented'}
                    </div>
                  </div>
                  ...
                  >
                    <svg width="20" height="20" fill={favorites.has(vehicles[0].id.toString()) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-2xl font-extrabold text-blue-900">{vehicles[0].name}</h3>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-2xl font-bold text-orange-500">{vehicles[0].price}</span>
                      ...
                      return (
                        <>
                          <div className="bg-gray-50 rounded-lg p-3 text-center">
                            <div className="text-lg font-bold text-blue-600">{vehicles[0].capacity}</div>
                            <div className="text-xs text-gray-500">Seats</div>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-3 text-center">
                            <div className="text-sm font-bold text-blue-600">{vehicles[0].transmission}</div>
                            <div className="text-xs text-gray-500">Transmission</div>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-3 text-center">
                            <div className="text-sm font-bold text-blue-600">{vehicles[0].fuel}</div>
                            <div className="text-xs text-gray-500">Fuel</div>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-3 text-center">
                            <div className="text-sm font-bold text-blue-600">{vehicles[0].year}</div>
                  ...
                  {/* Description */}
                  <p className="text-gray-600 mb-6 text-sm leading-relaxed">{vehicles[0].description}</p>
                  
                  {/* Action Buttons */}
                  <div className="flex gap-3">
                    <Link 
                      href={`/rent-a-car?vehicle=${vehicles[0].id}`} 
