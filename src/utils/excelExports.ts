...
const addAccountBalances = (sheet: ExcelJS.Worksheet, financialSummary: FinancialSummary): number => {
  ...
  const accounts = [
    ...
    { name: 'Cash', opening: financialSummary?.openingBalances?.cashRWF || 0, closing: financialSummary?.closingBalances?.cashRWF || 0 }
  ];
...
const addIncomeTransactions = (sheet: ExcelJS.Worksheet, financialSummary: FinancialSummary, startRow: number): number => {
  ...
  financialSummary?.income?.forEach((item) => {
    const total = (item.mtnMomoRWF || 0) + (item.equityBankRWF || 0) + (item.bkBankRWF || 0) + (item.cashRWF || 0);
    sheet.addRow([
      item.id,
      item.description,
      item.date,
